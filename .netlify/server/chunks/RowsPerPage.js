...
import { k as get_store_value, v as validate_store, b as subscribe } from "./lifecycle.js";
import { c as create_ssr_component, a as add_attribute, d as each, e as escape } from "./ssr.js";
...
const ThFilter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<th class="table-cell"><input class="input" type="text"${add_attribute("placeholder", placeholder, 0)}${add_attribute("value", value, 0)}></th>`;
});
const Search = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { handler } = $$props;
  let value = "";
  if ($$props.handler === void 0 && $$bindings.handler && handler !== void 0) $$bindings.handler(handler);
  return `<input class="input" placeholder="Search"${add_attribute("value", value, 0)}>`;
});
const RowsPerPage = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<aside>Show
   <select class="select" name="rowsPerPageOptions">${each(options, (option) => {
    return `<option class="option"${add_attribute("value", option, 0)}>${escape(option)} </option>`;
  })}</select>
   rows per page</aside>`;
});
export {
